...
| `excludedUnitTypes`              | `""`                 | Unit names to exclude from squad tracking (e.g. `"armrectr cornecro legrezbot"`). Applied immediately — the widget re-classifies and re-routes every unit (so you need to remake the squads). Use `add`/`remove` to build the list in smaller batches to stay within the chat length limit. You usually don't need to type unit names at all: select the units in-game and use the `squad_exclude_selected` / `squad_unexclude_selected` actions (see below). To find a unit's name by hand, open its page on [beyondallreason.info](https://www.beyondallreason.info) — the name is the last segment of the URL (e.g. `https://www.beyondallreason.info/unit/cornecro`, cornecro = Graverobber). |
...
| `idleColorBlendRate`             | `2`                  | Per-second rate at which a squad's hull crossfades between its active and idle color. No panel control — set via chat                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `squad_exclude_selected`                         | Adds the types of the currently selected units to the exclusion list, then re-classifies live |
| `squad_unexclude_selected`                       | Removes the types of the currently selected units from the exclusion list, then re-classifies live |
